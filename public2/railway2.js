        ...
        success : function(data, textStatus, jqXHR) {
            ...
            for(var i=0;i<data.length;i++)			  
			{
              ...
              var model =  window.schedules.get('trainId');
               ...
               //getTrainSchedule(data[i].train_uid);
...
function getTrainSchedule(uid){
    ...
    $.ajax({
        ...
        success : function(data, textStatus, jqXHR) {
			...
			console.log('latest schedule '+JSON.stringify(data));	
        },
		error:function(err){console.log(err)}
    }).done(function(){
	     console.log(' this query is done   ------------------------------------------- ');	
...
var icon = new google.maps.MarkerImage("TrainIcon1.png", new google.maps.Size(120, 120), new google.maps.Point(100, 34));
...
function loadArrival(city){ //  
	    $.getJSON('/railway/arrival?city='+city, function(data) {
            var items = []; 
			var n = 0;
			$('#timebody').html('');
...
function loadDeparture(city){  
        $.getJSON('/railway/departure?city='+city, function(data) {
            var items = []; 
			$('#timebody').html('');
			var n = 0;
            $.each(data, function(key, val) {
